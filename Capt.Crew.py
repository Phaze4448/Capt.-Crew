...
# --- ADD THIS LINE HERE TO INITIALIZE THE BOT VARIABLE ---
bot = commands.Bot(command_prefix="!", intents=intents)

# Load the dynamic server configuration name at the very top level
SERVER_NAME = os.getenv("SERVER_NAME", "Capt. Crew")
...
class CrewModel(BaseModel):
    ...
    description: str =f"Official {SERVER_NAME} Competitor Team."
...
async def on_ready():
    ...
    if SERVER_NAME ==f"{SERVER_NAME}":
        bot.db = client["sgt_crew_db"]
        bot.crew_prefix = "SGT. CREW"
        bot.dashboard_title =f"🎮 {SERVER_NAME} Matchmaking Dashboard"
        print(f"🟢 {SERVER_NAME} profile loaded!")
    else:
        bot.db = client["smash_crew_db"]
        bot.crew_prefix = "CREW"
        bot.dashboard_title = f"🎮 {SERVER_NAME} Matchmaking Dashboard"
        print(f"🔵 {SERVER_NAME} profile loaded! Initializing layout & [smash_crew_db]")

        
    await bot.change_presence(activity=discord.Game(name=f"{bot.crew_prefix} Battles"))
...
async def generate_player_card(username, fighter_name, background_name, tint_rgba, gold_balance, elo):
    ...
    try:
        base_bg = Image.open(io.BytesIO(bg_data)).convert("RGBA")
    except:
        base_bg = Image.new("RGBA", (800, 450), (20, 24, 28, 255)).resize((800, 450))
    try:
        fighter_img = Image.open(io.BytesIO(fighter_data)).convert("RGBA")
    except:
        fighter_img = Image.new("RGBA", (400, 400), (0, 0, 0, 0))
    ...
    return discord.FileW(fp=output_buffer, filename="smash_player_card.png")
...
async def show_card(interaction: discord.Interaction, member: discord.User = None):
    ...
    try:
        ...
        if profile_record:
            if "gold" in profile_record and profile_record["gold"] is not None:
                gold_balance = int(profile_record["gold"])
            if "unlocked_items" in profile_record and profile_record["unlocked_items"] is not None:
                unlocked_inventory = profile_record["unlocked_items"]
        ...
        if profile_record and profile_record.get("equipped_secondary"):
            equipped_secondary = str(profile_record["equipped_secondary"]).strip()
        if profile_record and profile_record.get("equipped_fighter"):
            equipped_fighter = str(profile_record["equipped_fighter"]).strip()

        # ADD THIS PULL STEP HERE:
        equipped_secondary = "None"
        if profile_record and profile_record.get("equipped_secondary"):
            equipped_secondary = str(profile_record["equipped_secondary"]).strip()
...
class RolePingSelect(Select):
    ...
    async def callback(self, interaction: discord.Interaction):
        ...
        # 1. Look up the interaction user's crew portfolio from MongoDB Atlas
        user_crew = await bot.db.crews.find_one({"members": interaction.user.id})
        
        if not user_crew:
            await interaction.followup.send("❌ **Access Denied:** You must belong to a registered crew roster to ping matching practice players.", ephemeral=True)
            return

        # 2. Extract the numeric member user ID array lists from the crew document
        crew_member_ids = user_crew.get("members", [])

        # 3. Filter pings: Must possess the role AND exist inside your MongoDB crew roster 
        to_ping = [
            m.mention for m in self.all_users 
            if m.id in crew_member_ids and any(r.name == target_role_name for r in m.roles)
        ]

        if to_ping:
            await interaction.followup.send(f"🚨 **{user_crew['name']} ({target_role_name}) Alert!** Match needed against **{self.fighter_name}**:\n" + " ".join(to_ping))
        else:
            await interaction.followup.send(f"ℹ No active **{self.fighter_name}** players inside your crew possess the **{target_role_name}** role framework tier.", ephemeral=True)

    async def single_ping_callback(self, interaction: discord.Interaction):
        await interaction.response.defer()
        target_id = int(interaction.data['values'][0])
        member = interaction.guild.get_member(target_id)
        
        if not member:
            await interaction.followup.send("❌ Could not identify that user on the server roster.", ephemeral=True)
            return

        # 1. Fetch the challenger's active crew profile from MongoDB
        user_crew = await bot.db.crews.find_one({"members": interaction.user.id})
        
        if not user_crew:
            await interaction.followup.send("❌ **Access Denied:** You must belong to a registered crew roster to issue a direct challenge.", ephemeral=True)
            return

        # 2. Guard Check: Verify if the challenged player exists in the same crew list array
        crew_member_ids = user_crew.get("members", [])
        if member.id not in crew_member_ids:
            await interaction.followup.send(f"❌ **Matchmaking Blocked:** {member.display_name} is not part of your crew roster context.", ephemeral=True)
            return

        # 3. Success: Both are confirmed squad members
        await interaction.followup.send(f"🔔 **[{user_crew['name']}]** {interaction.user.mention} has formally challenged squad teammate {member.mention} to practice matchups against **{self.fighter_name}**!")
...
class RolePingSelect(Select):
    ...
    async def single_ping_callback(self, interaction: discord.Interaction):
        ...
        # 1. Fetch the challenger's active crew profile from MongoDB Atlas
        user_crew = await bot.db.crews.find_one({"members": interaction.user.id})
...
class RosterView(View):
    ...
    async def select_callback(self, interaction: discord.Interaction):
        try:
            # 1. Defend the thread from timeout gates instantly
            ...
            # 2. Fetch the user's crew data securely from MongoDB Atlas
            user_crew = await bot.db.crews.find_one({"members": interaction.user.id})
            if not user_crew:
                await interaction.followup.send("❌ **Access Denied:** You must belong to a registered crew roster to access crew matching pings.", ephemeral=True)
                return
                
            crew_member_ids = user_crew.get("members", [])

            # 3. Filter server roles to isolate matching players inside your crew roster array
            mains_role = discord.utils.get(guild.roles, name=f"Main: {name}")
            mains = [m for m in mains_role.members if m.id in crew_member_ids] if mains_role else []
            
            secs_role = discord.utils.get(guild.roles, name=f"Secondary: {name}")
            secs = [s for s in secs_role.members if s.id in crew_member_ids] if secs_role else []
            
            all_p = list(set(mains + secs))

            # 4. Generate the private player menu text summary interface
            text = f"🔲 **[{user_crew.get('name', 'Crew')}] Fighter Profile: {name}**\n\n⭐ **Mains:**\n" + ("\n".join([f"• {m.display_name}" for m in mains]) if mains else "• None\n")
            text += f"\n🥈 **Secondaries:**\n" + ("\n".join([f"• {s.display_name}" for s in secs]) if secs else "• None\n")

            # 5. Build dynamic components natively on a self-contained View layer
            dynamic_view = discord.ui.View(timeout=None)
            
            if all_p:
                # --- ITEM A: Targeted Selection Dropdown (Max 25 options) ---
                opts = [
                    discord.SelectOption(label=p.display_name, value=str(p.id), description="Main" if p in mains else "Secondary") 
                    for p in all_p[:25]
                ]
                single_sel = discord.ui.Select(placeholder="🎯 Select 1 Specific Player to Ping...", options=opts, row=0)
                
                async def dynamic_ping_callback(select_interaction: discord.Interaction):
                    target_id = int(select_interaction.data['values'][0])
                    member = select_interaction.guild.get_member(target_id)
                    if member:
                        await select_interaction.response.send_message(f"🔔 **[{user_crew.get('name', 'Crew')}]** {select_interaction.user.mention} tagged crewmate {member.mention} regarding **{name}**!")
                        
                single_sel.callback = dynamic_ping_callback
                dynamic_view.add_item(single_sel)

                # --- ITEM B: The Crew-Locked Global Ping Button ---
                btn_everyone = discord.ui.Button(label="Ping Everyone who plays character", style=discord.ButtonStyle.danger, row=1)
                
                async def dynamic_button_callback(btn_interaction: discord.Interaction):
                    # Gather and structure mentions safely for matching teammates
                    mentions_string = " ".join([m.mention for m in all_p])
                    await btn_interaction.response.send_message(f"📣 **[{user_crew.get('name', 'Crew')}]** {btn_interaction.user.mention} requested support from all **{name}** players:\n{mentions_string}")
                    
                btn_everyone.callback = dynamic_button_callback
                dynamic_view.add_item(btn_everyone)
            else:
                text += "\nℹ️ *No active members of your crew currently have this character registered to their roles.*"

            # 6. Dispatch the summary interface response layout flawlessly
            await interaction.followup.send(content=text, view=dynamic_view, ephemeral=True)

        except Exception as e:
            print(f"🚨 MATCH MENU CRASH: {e}")
            import traceback
            traceback.print_exc()
            try:
                await interaction.followup.send(f"⚠️ **Interactive Menu Error:** `{str(e)}`", ephemeral=True)
            except:
                pass



@bot.tree.command(name="instantsync", description="Admin Only: Force clear local Discord server UI cache and push advanced features.")
@app_commands.checks.has_permissions(administrator=True)
async def manual_sync(interaction: discord.Interaction):
    # Defend the connection from timeout gates instantly
    await interaction.response.defer(ephemeral=True)
    
    # Force copy global features directly over this guild's layer override lists
    bot.tree.copy_global_to(guild=interaction.guild)
    synced = await bot.tree.sync(guild=interaction.guild)
    
    await interaction.followup.send(f"⚡ **Instant Cache Flush Complete!** Synced {len(synced)} specialized modules straight to this guild zone layout.", ephemeral=True)
...
async def start_battle(interaction: discord.Interaction, opponent_crew: str):
    ...
    battle_channel = await guild.create_text_channel(
        ...
        topic=f"Official {SERVER_NAME} Match Hub: {crew_a_data['name']} vs {crew_b_data['name']}"
    )
    ...
    embed = discord.Embed(title=f"⚔️ {SERVER_NAME} Arena Arena Online ⚔️", color=discord.Color.red())
...
@bot.command(name="instant_sync")
@commands.is_owner()
async def manual_sync(ctx):
    # This targets your current server directly, clearing Discord's UI cache instantly
    bot.tree.copy_global_to(guild=ctx.guild)
    synced = await bot.tree.sync(guild=ctx.guild)
    await ctx.send(f"⚡ **Instant Cache Flush Complete!** Synced {len(synced)} specialized modules straight to this guild zone layout.")
...
async def leaderboard(interaction: discord.Interaction):
    ...
    embed = discord.Embed(title=f"🏆 {SERVER_NAME} Seasonal Standings", color=discord.Color.gold())
    for i, c in enumerate(crews, 1):
        embed.add_field(name=f"#{i} {c.get('name', 'Unknown Crew')}", value=f"Elo: `{c.get('elo', 1000)}` | Record: {c.get('wins', 0)}W - {c.get('losses', 0)}L", inline=False)
...
async def approve_battle(interaction: discord.Interaction, winner_crew: str, loser_crew: str):
    ...
    # [Paste this right inside your approve_battle command where your stats increments conclude]
    # Update Player Line-Graph Historical Trajectory logs
    battle_data = await bot.db.active_battles.find_one({"channel_id": interaction.channel_id})
    if battle_data:
        # Log entries for roster participants to generate line graph series targets later
        for p_id in battle_data.get("roster_a", []) + battle_data.get("roster_b", []):
            await bot.db.match_logs.insert_one({
                "player_id": p_id,
                "stocks_taken": 2, # Captures default baseline, hooks up to score parse lines
                "timestamp": datetime.utcnow()
            })
            
        # Distribute Spectator Betting Gold Pools dynamically
        wagers = battle_data.get("spectator_wagers", [])
        for wager in wagers:
            if wager["crew"] == winner_crew.strip().lower():
                # Double their investment upon accurate tracking validation
                payout = wager["amount"] * 2
                await bot.db.users.update_one({"_id": wager["user_id"]}, {"$inc": {"gold": payout}}, upsert=True)
                print(f"🎰 Spectator Payout: Disbursed {payout}G to user ID {wager['user_id']}")
...
async def stagelist(interaction: discord.Interaction):
    embed = discord.Embed(title=f"🗺️📖 Official {SERVER_NAME} Arena Ruleset", color=discord.Color.dark_gray())
...
async def setmains(interaction: discord.Interaction, mains: str):
    ...
    # Clean input and write directly to your card rendering target field
    clean_main = mains.strip().title()
    await bot.db.users.update_one(
        {"_id": user_id},
        {"$set": {"equipped_fighter": clean_main}},
        upsert=True
    )
    await interaction.followup.send(f"🎯 Your competitive primary fighter choice has been saved as **{clean_main}**!", ephemeral=True)
...
async def setsecondary(interaction: discord.Interaction, alternate_char: str):
    ...
    # Clean input and write directly to your card secondary tracking field
    clean_secondary = alternate_char.strip().title()
    await bot.db.users.update_one(
        {"_id": user_id},
        {"$set": {"equipped_secondary": clean_secondary}},
        upsert=True
    )
    await interaction.followup.send(f"🥈 Your pocket backup secondary selection has been saved as **{clean_secondary}**!", ephemeral=True)
...
async def help_referee(interaction: discord.Interaction):
    embed = discord.Embed(title=f"📖 {SERVER_NAME}: Referee Manual", color=discord.Color.blue())
    embed.description = "Core flow chart steps mapping match commands out sequentially for server tournament structures."
    await interaction.response.send_message(embed=embed)


if __name__ == "__main__":
    print("🚀 System initializing... Connecting bot to Discord Gateway...")
    
    # Grab the active environment token loaded via python-dotenv
    token = os.getenv("DISCORD_TOKEN")
    
    if token:
        bot.run(token)
    else:
        print("❌ CRITICAL ERROR: DISCORD_TOKEN is missing from your environment setup.")
class RosterView(View):
    def __init__(self, page=1):
        super().__init__(timeout=None)
        self.page = page
    ...
    async def select_callback(self, interaction: discord.Interaction):
        ...
        # 1. Look up the command caller's active crew profile from MongoDB Atlas
        user_crew = await bot.db.crews.find_one({"members": interaction.user.id})
        
        if not user_crew:
            await interaction.followup.send("❌ **Access Denied:** You must be a registered member of a crew to pull roster menu lists.", ephemeral=True)
            return
            
        crew_member_ids = user_crew.get("members", [])

        # 2. Query guild cosmetic roles but restrict members strictly to your crew ID matrix list
        mains_role = discord.utils.get(guild.roles, name=f"Main: {name}")
        mains = [m for m in mains_role.members if m.id in crew_member_ids] if mains_role else []
        ...
        # 3. Build selection options restricted to squad teammates
        ...
        # 4. Generate the private player menu interface view channel logs
        text = f"🔲 **[{user_crew['name']}] Fighter Profile: {name}**\n\n⭐ **Mains:**\n" + ("\n".join([f"• {m.display_name}" for m in mains]) if mains else "• None\n")
        text += f"\n🥈 **Secondaries:**\n" + ("\n".join([f"• {s.display_name}" for s in secs]) if secs else "• None\n")
...
if __name__ == "__main__":
    ...
    if token:
        ...
        print("❌ CRITICAL ERROR: DISCORD_TOKEN is missing from your environment setup.")
# =========================================================================
# ⚙️ CREW LEAGUE GRAND SCALE EVOLUTIONARY EXTENSION MODULE
# =========================================================================
import matplotlib
matplotlib.use('Agg')  # Force headless rendering mode to prevent thread crashes
import matplotlib.pyplot as plt
from datetime import datetime

# --- 🎒 FEATURE 1 & 2: RE-ENGINEERED ECO/Consumables LOOP ---
# Append functional assets directly into your marketplace system registry maps
FUNCTIONAL_SHOP = {
    "scouting drone": {"cost": 400, "desc": "Consumable: Use before a match to strip hidden blind pick locks."},
    "pocket stadium card": {"cost": 500, "desc": "Consumable: Instantly clear one stage strike your rival just placed."}
}

@bot.tree.command(name="shop_advanced", description="Browse specialized tactical match consumables.")
async def advanced_shop(interaction: discord.Interaction):
    await interaction.response.defer(ephemeral=True)
    embed = discord.Embed(title="🎒 Tactical Consumables Marketplace", color=discord.Color.dark_gold())
    for item, data in FUNCTIONAL_SHOP.items():
        embed.add_field(name=f"📦 {item.title()} ({data['cost']}G)", value=data['desc'], inline=False)
    await interaction.followup.send(embed=embed, ephemeral=True)

@bot.tree.command(name="buy_advanced", description="Purchase a functional tactical match consumable.")
async def buy_advanced(interaction: discord.Interaction, item: str):
    await interaction.response.defer(ephemeral=True)
    norm_item = item.strip().lower()
    if norm_item not in FUNCTIONAL_SHOP:
        await interaction.followup.send("❌ That tactical accessory is missing from our current manufacturing listings.", ephemeral=True)
        return
    
    cost = FUNCTIONAL_SHOP[norm_item]["cost"]
    user_data = await bot.db.users.find_one({"_id": interaction.user.id})
    current_gold = user_data.get("gold", 0) if user_data else 0
    
    if current_gold < cost:
        await interaction.followup.send(f"❌ Transaction Declined: Required {cost}G, you currently possess {current_gold}G.", ephemeral=True)
        return
        
    await bot.db.users.update_one(
        {"_id": interaction.user.id},
        {"$inc": {"gold": -cost}, "$push": {"consumables": norm_item}},
        upsert=True
    )
    await interaction.followup.send(f"🎉 **Purchase Confirmed:** Added 1x **{item.title()}** to your inventory tactical matrix!", ephemeral=True)

# --- 📉 FEATURE 3: CAREER STOCK TRAJECTORY TREND GRAPHS ---
# Historical logging structure tracking match parameters over linear time frames
@bot.tree.command(name="trends", description="Render an analytics line graph tracking your recent performance trajectory.")
async def career_trends(interaction: discord.Interaction, player: discord.Member = None):
    await interaction.response.defer()
    target = player or interaction.user
    
    # Query historic chronologically ordered logging matches
    cursor = bot.db.match_logs.find({"player_id": target.id}).sort("timestamp", 1).limit(10)
    history = await cursor.to_list(length=10)
    
    if not history or len(history) < 2:
        await interaction.followup.send(f"ℹ️ Insufficient data metrics found. **{target.display_name}** must log at least 2 match records.")
        return
        
    stocks_taken_series = [log.get("stocks_taken", 0) for log in history]
    match_indices = [f"M{i}" for i in range(1, len(history) + 1)]
    
    # Initialize head-less matplotlib drawing frames safely
    plt.figure(figsize=(7, 4))
    plt.plot(match_indices, stocks_taken_series, marker='o', color='#7289DA', linewidth=2, label="Stocks Taken")
    plt.title(f"Career Performance Trajectory: {target.name}", color='white', fontsize=12)
    plt.xlabel("Recent Chronological Matches", color='gray')
    plt.ylabel("Stocks Extracted Per Set", color='gray')
    plt.grid(True, linestyle='--', alpha=0.3)
    
    # Style styling coordinates matching discord's presentation template maps
    fig = plt.gcf()
    ...
    file = discord.File(fp=buf, filename="performance_trajectory.png")
    await interaction.followup.send(f"📊 **Career Performance Analytics Tracker for {target.mention}:**", file=file)

# --- 🏟 FEATURE 4 & 5: STATE REGIONAL DOMINANCE MATRIX & CONQUESTS ---
@bot.tree.command(name="regions", description="Display collective regional dominance scoreboards.")
async def regional_scoreboard(interaction: discord.Interaction):
    await interaction.response.defer()
    # Execute database aggregate chains to calculate collective regional performance metrics
    pipeline = [
        {"$group": {
            "_id": "$region",
            "total_stocks": {"$sum": "$stocks_taken"},
            "mvp_count": {"$sum": "$mvps"},
            "total_players": {"$sum": 1}
        }},
        {"$sort": {"total_stocks": -1}}
    ]
    cursor = bot.db.players.aggregate(pipeline)
    regions = await cursor.to_list(length=10)
    
    embed = discord.Embed(title="🗺 Regional Territory Dominance Standings", color=discord.Color.blue())
    for i, reg in enumerate(regions, 1):
        reg_name = reg["_id"] if reg["_id"] else "Free Territories"
        embed.add_field(
            name=f"#{i} Region: {reg_name.upper()}",
            value=f"⚔️ **Total Stocks Extracted:** `{reg['total_stocks']}`\n👥 **Active Combatants:** `{reg['total_players']}`\n🎖 **MVPs Crowned:** `{reg['mvp_count']}`",
            inline=False
        )
    await interaction.followup.send(embed=embed)

# --- 🎰 FEATURE 6 & 7: SPECTATOR MATCH WAGER LABELS ---
@bot.tree.command(name="wager", description="Bet your profile gold on an active ongoing league crew battle.")
async def place_wager(interaction: discord.Interaction, crew_selection: str, gold_amount: int):
    await interaction.response.defer(ephemeral=True)
    
    # Verify the wager target belongs inside an active ongoing battle channel tracking thread
    battle = await bot.db.active_battles.find_one({"channel_id": interaction.channel_id})
    if not battle:
        await interaction.followup.send("❌ Error: You can only place spectator wagers inside an active match arena channel.", ephemeral=True)
        return
        
    if gold_amount <= 0:
        await interaction.followup.send("❌ Error: Invalid transaction volume argument values.", ephemeral=True)
        return
        
    user_data = await bot.db.users.find_one({"_id": interaction.user.id})
    current_gold = user_data.get("gold", 0) if user_data else 0
    if current_gold < gold_amount:
        await interaction.followup.send(f"❌ Betting Declined: Insufficient balance. Wallet: {current_gold}G.", ephemeral=True)
        return
        
    # Commit the spectator bet object directly inside the active match row document structure arrays
    wager_obj = {"user_id": interaction.user.id, "crew": crew_selection.strip().lower(), "amount": gold_amount}
    await bot.db.active_battles.update_one(
        {"channel_id": interaction.channel_id},
        {"$push": {"spectator_wagers": wager_obj}}
    )
    # Hold the transaction amount out of their active balance ledger
    await bot.db.users.update_one({"_id": interaction.user.id}, {"$inc": {"gold": -gold_amount}})
    await interaction.followup.send(f"🎰 **Wager Registered Successfully:** Risked **{gold_amount}G** on **{crew_selection.upper()}**!", ephemeral=True)

# --- 🎭 FEATURE 8: AUTOMATED ALGORITHMIC RIVALRY WEEK MATH HOOK ---
@bot.tree.command(name="scout_rivalries", description="Staff Only: Analyze database metrics to uncover deadlocked grudges.")
@app_commands.checks.has_permissions(administrator=True)
async def algorithmic_scout(interaction: discord.Interaction):
    await interaction.response.defer()
    cursor = bot.db.crews.find()
    all_crews = await cursor.to_list(length=50)
    
    suggestions = []
    # Nested iteration evaluation structures checking Elo differential spreads matching rival patterns
    for i, crew_a in enumerate(all_crews):
        for crew_b in all_crews[i+1:]:
            elo_diff = abs(crew_a["elo"] - crew_b["elo"])
            # If two crews sit within a tight 20 Elo bounds, trigger a competitive match suggestion alert
            if elo_diff <= 20:
                suggestions.append(f"⚡ **{crew_a['name']}** (Elo: {crew_a['elo']}) vs **{crew_b['name']}** (Elo: {crew_b['elo']}) | Spread Margin: `{elo_diff} pts`")
                
    embed = discord.Embed(title="🛡 Algorithmic Rivalry Suggestions Engine", color=discord.Color.red())
    embed.description = "\n".join(suggestions) if suggestions else "ℹ️ No deadlocked organizational rating alignments found currently."
    await interaction.followup.send(embed=embed)
